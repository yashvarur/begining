...
long myioctl(struct file *fp,unsigned int pid, unsigned long usr_vaddr)
{
    ...
    printk("normal_priority = %d\n",ptr -> normal_prio);
    printk("normal_priority = %d\n",ptr -> prio);
    printk("policy = %d\n",ptr -> policy);
    switch(ptr -> policy) {
	case SCHED_NORMAL : printk("normal value\n");
			    break;
	case SCHED_FIFO : printk("FIFO policy\n");
			    break;
	case SCHED_RR : printk("FIFO policy\n");
			    break;
	case SCHED_BATCH : printk("BATCH");
			   break;
   }
}
static void myExit(void)
