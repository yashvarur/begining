...
struct super_block { /* superblock structure */
...
struct inode_table { /* Inode table*/
    unsigned int i_no;
    unsigned int size;
...
void create_file(int *ip, struct inode_table *itp, struct super_block *sup)
{
    ...
    do {
    	...
    	*(file + strlen(file) - 1) = '\0';
    	if(strlen(file) > 8) {
		printf("file name is too long maximum character is 8\n");
		printf("Try Again........\n");
	}		
    }while(strlen(file) > 8);
	...
	*(file + strlen(file) - 1) = '\0';
	...
	if (k == 0)
	    printf("file does not exist\n");
			...
			read_content = (char *) malloc( 1000000 * 1024);
			printf("enter content of file\n");
   			__fpurge(stdin);
			//size = read(0, read_content, 1024 * 5);
			fgets(read_content,1000000*1024,stdin);
			*(read_content + strlen(read_content) -1) = '\0';
			...
			if(temp > 5) {
				printf("size of file is large maximum block allowed is 5\n");
				return;
			}
			for (j = 0; j <= 31; j++) {
				(sup->free_datablocks)--;
